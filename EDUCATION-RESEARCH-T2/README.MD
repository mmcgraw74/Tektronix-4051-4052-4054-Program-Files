...
I was able to recover the 3D Function Plot program and created a separate directory for that program with a screenshot of running the program and the program instructions for the newer General Function Plot program into this directory: 
...
You might notice I moved my Cubic Spline program in the DIRECTORY menu to 11 - as I edited the program to change the file numbers from 1, 2 and 3 to 24, 25 and 26 and changed the tape commands to Flash Drive commands - so this program works in 4050 computers and the 4051 Emulator without moving all the files to a separate tape.

However - I do not have the EDUCATION-RESEARCH T2 instructions manual for any of these programs - including my Cubic Spline program.  Some of the programs PRINT instructions at the beginning, but most of them do not have instructions - other than comment statements in the program.

The DIRECTORY menu programs 13 and above have multiple program files (like my Cubic Spline program) to reduce the RAM requirements of the program - since the original Tektronix 4051 computer was introduced in 1975 with only 8KB of RAM and 32KB BASIC ROM.  Tektronix 4050 BASIC has an APPEND command to append another program file into RAM at a designated program line number.  Typically you would delete some of the program lines and append a different set of lines into your program - for example: replace one subroutine with a different subroutine.
